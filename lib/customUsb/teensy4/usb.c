...
// CUSTOM
volatile uint8_t custom_feature_buffer[64] __attribute__ ((aligned(32)));
...
static void endpoint0_setup(uint64_t setupdata)
{
	...
	case 0x0921: // HID SET_REPORT
		// Check if this is a Feature Report (0x03) for Report ID 0x01
		// wValue: High Byte = Report Type (0x03), Low Byte = Report ID (0x01)
		if (setup.wValue == 0x0301 && setup.wLength <= 64) {
			endpoint0_setupdata.bothwords = setup.bothwords;
			endpoint0_receive(custom_feature_buffer, setup.wLength, 1);
			return;
		}
		// Fallback for standard 8-byte reports (like Keyboard LEDs)
...
static void endpoint0_complete(void)
{
	...
	// Handle the standard LED report (usually setup.word1 == 0x02000921)
	if (setup.word1 == 0x02000921 && setup.word2 == ((1 << 16) | KEYBOARD_INTERFACE)) {
		keyboard_leds = endpoint0_buffer[0];
		endpoint0_transmit(NULL, 0, 0);
	}

	// ADD THIS: Handle our "Secret" Feature Report
	// 0x03010921 corresponds to: SET_REPORT (0x09), Type Feature (0x03), ID (0x01), Class/Interface (0x21)
	if (setup.word1 == 0x03010921 && setup.wIndex == KEYBOARD_INTERFACE) {
		// On Teensy 4.1, we must invalidate the cache to see the data written by DMA
		arm_dcache_delete(custom_feature_buffer, 64);

		custom_feature_data_ready = 1; // Set flag for the main sketch
		endpoint0_transmit(NULL, 0, 0); // Send ZLP ACK to host
	}
#endif
